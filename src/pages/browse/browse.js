...
// 'date' = χρονολογικά κατά doc_date (προεπιλογή, ήδη η σειρά που έρχεται από το backend).
// 'recent' = κατά created_at (πότε μπήκε η εγγραφή στη βάση) — ελεύθερη εναλλαγή, καμία
// αυτόματη λογική/timer, βλ. συζήτηση 2026-09-26.
let browseSortMode = 'date';
...
function applyBrowseSearch(rows) {
  ...
  filtered = filtered.slice().sort((a, b) => {
    ...
    if (browseSortMode === 'recent') return (b.created_at || '').localeCompare(a.created_at || '');
    return 0; // ήδη ταξινομημένα κατά doc_date DESC από το backend, σταθερό sort διατηρεί τη σειρά
  });
...
document.getElementById('browse-sort-toggle-btn').addEventListener('click', () => {
  browseSortMode = browseSortMode === 'date' ? 'recent' : 'date';
  const btn = document.getElementById('browse-sort-toggle-btn');
  btn.textContent = browseSortMode === 'recent' ? '🕐 Πρόσφατα' : '📅 Χρονολογικά';
  applyBrowseSearch(browseRows);
});
